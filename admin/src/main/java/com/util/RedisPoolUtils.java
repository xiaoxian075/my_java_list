...
/**************************** redis 列表List start***************************/
	...
	public static boolean hmset(String key, String[] fields, String[] values){
		if(StringUtils.isBlank(key) || /*StringUtils.isEmptyArray(fields)*/fields==null || fields.length==0 || /*StringUtils.isEmptyArray(values)*/values==null || values.length==0){
			return false;
		}
		Jedis jedis = jedisPool.getResource();
	...
	public static boolean hmset(String key, Map<String, String> map){
		if(StringUtils.isBlank(key) || /*StrUtils.isEmptyMap(map)*/map==null || map.size()==0){
			return false;
		}
		Jedis jedis = jedisPool.getResource();
	...
	public static boolean hdel(String key, String... fields){
		if(StringUtils.isBlank(key)/* || StrUtils.isNull(fields)*/){
			return false;
		}
		for(String s : fields) {
			if (s==null)
